...
| `world-state-store` | string | Optional | Name of a `rdk:service:world_state_store` service (e.g. the `viam:world-state-aggregator:default` module). When set: (1) after every successful `switch_tool` the tool-changer publishes a `Transform` describing the newly-attached tool (geometry, pose relative to `attach-frame`) via `{"set_transform": ...}` DoCommand, and on `release` publishes `{"remove_transform": ...}` for the same UUID (`tool-changer/<service-name>/attached`); (2) at plan time the tool-changer reads current transforms from the store (dropping its own `attached` UUID) and merges them into the WorldState passed to the motion planner. Each planning step also includes the appropriate attached-tool geometry so the planner sees the tool during slide-out / transit-out of a take and transit-in / slide-in of a release. Publish failure after a successful physical swap returns an error but `currentTool` is still updated so state stays consistent. Only tools with `geometry` set produce publishes. |
...
| `geometry` | object | Optional | Viam `spatialmath.GeometryConfig` describing the tool's physical shape. When set together with `attach-frame`, the tool-changer publishes this geometry to the configured `world-state-store` while the tool is attached so motion planners see the correct occupancy. |
| `attach-frame` | string | Conditional | Frame this tool hangs off when attached (e.g. `gripper-1`). Required when `geometry` is set. |
| `attach-pose` | object | Optional | Pose of the tool's own frame relative to `attach-frame`. Defaults to identity. `orientation` is required if provided. |
