...
| `world-state-store` | string | Optional | Name of a `rdk:service:world_state_store` service (e.g. the `viam:world-state-aggregator:default` module). When set, after every successful `switch_tool` the tool-changer publishes a `Transform` describing the newly-attached tool (geometry, pose relative to `attach-frame`) via `{"set_transform": ...}` DoCommand. On `release`, it publishes `{"remove_transform": ...}` for the same UUID (`tool-changer/<service-name>/attached`). Publish failure after a successful physical swap returns an error but `currentTool` is still updated so state stays consistent. Only tools with `geometry` set produce publishes. |
